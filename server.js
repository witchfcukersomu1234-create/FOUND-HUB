...
app.post('/api/items/:id/claims', auth, (req,res)=>{
  const {message=''}=req.body; const it=db.prepare('SELECT * FROM items WHERE id=?').get(req.params.id);
  if(!it) return res.status(404).json({error:'Item not found'}); if(it.owner_id===req.user.id) return res.status(400).json({error:'Cannot claim your own post'});
...
app.post('/api/claims/:id/decision', auth, (req,res)=>{
  const {decision}=req.body; const cl=db.prepare('SELECT * FROM claims WHERE id=?').get(req.params.id); if(!cl) return res.status(404).json({error:'Claim not found'});
  const it=db.prepare('SELECT * FROM items WHERE id=?').get(cl.item_id); if(!(ownerOrAdmin(req.user.id,it.owner_id,req.user.role))) return res.status(403).json({error:'Forbidden'});
  db.prepare('UPDATE claims SET status=? WHERE id=?').run(decision,cl.id); if(decision==='approved'){ db.prepare('UPDATE items SET status="claimed" WHERE id=?').run(it.id); }
  res.json(db.prepare('SELECT * FROM claims WHERE id=?').get(cl.id));
});
