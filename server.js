...
const ownerOrAdmin = (userId, ownerId, role) =>
...
app.post('/api/items', auth, upload.single('photo'), (req, res) => {
  const { type, title, description = '', category = '', location = '' } =
    req.body;
  ...
  const info = db
    .prepare(
      `INSERT INTO items (type,title,description,category,location,photo_url,owner_id)
       VALUES (?,?,?,?,?,?,?)`
    )
    .run(
      type,
...
app.get('/api/items', (req, res) => {
  ...
  let sql =
    'SELECT items.*, users.name AS owner_name FROM items JOIN users ON users.id=items.owner_id WHERE 1=1';
  const p = [];
  ...
  if (q) {
    sql +=
      ' AND (title LIKE ? OR description LIKE ? OR location LIKE ? OR category LIKE ?)';
    p.push(`%${q}%`, `%${q}%`, `%${q}%`, `%${q}%`);
  }
...
app.patch('/api/items/:id', auth, (req, res) => {
  const it = db
    ...
    .get(req.params.id);
  if (!it) return res.status(404).json({ error: 'Not found' });
  if (!ownerOrAdmin(req.user.id, it.owner_id, req.user.role))
  ...
  res.json(
    db.prepare('SELECT * FROM items WHERE id=?').get(req.params.id)
  );
});
...
app.post('/api/items/:id/claims', auth, (req, res) => {
  ...
  if (it.owner_id === req.user.id)
    return res
      .status(400)
      .json({ error: 'Cannot claim your own post' });
  ...
  res.json(
    db.prepare('SELECT * FROM claims WHERE id=?').get(info.lastInsertRowid)
  );
});
...
app.get('/api/my/items', auth, (req, res) => {
  res.json(
    db
      .prepare(
        'SELECT * FROM items WHERE owner_id=? ORDER BY date_reported DESC'
      )
      .all(req.user.id)
  );
});
...
app.post('/api/claims/:id/decision', auth, (req, res) => {
  ...
  res.json(
    db.prepare('SELECT * FROM claims WHERE id=?').get(cl.id)
  );
});
